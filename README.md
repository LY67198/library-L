...
├── agents/                   # 图书馆 Agent 层
├── backend/
│   ├── router/               # chat, book, auth, seat
│   ├── schemas/              # chat, auth, seat
│   └── service/              # chat, auth, seat
├── core/                     # database, security, deps, lock
├── models/                   # User, Floor, Zone, Seat, SeatTimeSlot, Appointment
├── research_agents/          # LLMClient + RuleBasedLLMClient
└── app_main.py
...
| 2 | 记忆 | `app/research_agents/memory/` | 实现 `MemoryStore`，接入 Redis/Postgres |
| 3 | 子图 | `app/agents/` | 画像子图从 stub 升级 |
