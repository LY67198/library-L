...
> **Phase 1 已完成** — AI 智能问答 + 馆藏检索，32 tests passed。后续 Phase 内容仍在规划中。
...
| POST | `/api/v1/auth/register` | 用户注册 | 🔜 Phase 2 |
| POST | `/api/v1/auth/login` | 用户登录 | 🔜 Phase 2 |
| GET | `/api/v1/auth/me` | 当前用户信息 | 🔜 Phase 2 |
| GET | `/api/v1/seats/` | 座位列表 | 🔜 Phase 2 |
| POST | `/api/v1/seats/book` | 预约座位 | 🔜 Phase 2 |
| GET | `/api/v1/appointments/` | 我的预约 | 🔜 Phase 2 |
| POST | `/api/v1/appointments/{id}/cancel` | 取消预约 | 🔜 Phase 2 |
...
- **Phase 2** — 用户系统 + 座位预约 + Redis 分布式锁 + Celery + Alembic 数据库迁移
